...
validate_node_id <- function(node_id) {
  if (!is.numeric(node_id) || length(node_id) != 1 || is.na(node_id) || node_id != as.integer(node_id)) {
...
validate_weight <- function(weight) {
  if (!is.numeric(weight) || length(weight) != 1 || is.na(weight) || weight < 0) {
    stop("Weight must be a single non-negative numeric value, got: ", deparse(weight))
  }
  as.numeric(weight)
}
  ...
  public = list(
    ...
    add_node = function(id) {
      ...
      if (!exists(id_str, envir = self$node_to_index)) {
        ...
        self$adj_list[[length(self$nodes_vec)]] <- list(targets = integer(0), weights = numeric(0))
      }
      invisible(self)
    },
    ...
    add_edge = function(source, target, weight = DEFAULT_WEIGHT) {
      ...
      if (source == target) {
        stop("Self-loops are not supported: source=", source, ", target=", target)
      }
      ...
      if (!is.na(target_pos)) {
        ...
        self$adj_list[[source_idx]]$weights <- c(self$adj_list[[source_idx]]$weights, weight)
...
relief_rank <- function(graph, inv_graph, max_iter = MAX_ITERATIONS, progress = TRUE) {
  if (length(graph) == 0) {
  ...
  while (iteration < max_iter) {
    ...
    if (max_slack <= TOLERANCE) {
      if (progress) {
        cat("Converged: maximum slack ≤ tolerance at iteration", iteration, "\n")
      }
      break
    }
    ...
    if (!is.finite(agony_diff) || abs(agony_diff) < TOLERANCE) {
      ...
      if (no_improvement_count >= 5) {
        if (progress) {
          cat("Converged: no improvement for 5 iterations at iteration", iteration, "\n")
        }
        break
      }
    } else {
    ...
    if (is.finite(current_agony) && is.finite(previous_agony) && 
...
unweighted_agony_lp <- function(graph, progress = TRUE) {
  ...
  if (result$status != 0) {
    stop("LP solver failed with status: ", result$status, 
...
load_edgelist <- function(filename, progress = TRUE) {
  ...
  for (i in seq_len(nrow(edges))) {
    ...
    first_col <- row[[1]]
    if (is.na(first_col) || nchar(trimws(first_col)) == 0 || 
    ...
    if (ncol(row) >= 2 && !is.na(row[[2]])) {
      ...
      tryCatch({
        ...
        if (ncol(row) >= 3 && !is.na(row[[3]]) && nchar(trimws(row[[3]])) > 0) {
  ...
  if (progress) {
    cat("Loaded", valid_edges, "valid edges into graph with", 
...
compute_agony <- function(graph, method = c("lp", "relief"), 
                         progress = TRUE, max_iter = MAX_ITERATIONS) {
  ...
  if (progress) {
    cat("Computing agony for graph with", graph$num_nodes(), 
        "nodes and", graph$num_edges(), "edges\n")
  }
